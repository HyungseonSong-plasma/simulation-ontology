    ...
    fn sort_key(&self) -> String {
        match self {
            ...
            Self::Relation {
                ...
                target,
                ..
            } => format!("relation:{source}:{relation_kind:?}:{target}"),
        }
    }
}
...
impl MappingClaimDto {
    fn normalize(&mut self) -> Result<(), RealizationDtoError> {
        ...
        self.evidence.sort_by(|left, right| {
            evidence_key(left)
                .cmp(&evidence_key(right))
        });
        self.evidence.dedup();

        if let Some(provenance) = &self.provenance {
            if provenance.producer.is_empty() {
                return Err(RealizationDtoError::EmptyField("mapping provenance producer"));
            }
...
pub fn compare_subject_and_effect(
    ...
    if intended_subject != &effect.subject {
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::CycleDetected(actions) => {
                write!(formatter, "mapping plan cycle detected among: {}", actions.join(", "))
            }
...
fn reject_backend_native_fields(extensions: &Extensions) -> Result<(), RealizationDtoError> {
    ...
    for field in FORBIDDEN {
        if extensions.contains_key(*field) {
            return Err(RealizationDtoError::BackendNativeLeakage((*field).to_owned()));
        }
    }
    Ok(())
}
