...
/// Executes the published Adapter Protocol 0.1 positive fixture baseline through an
/// external adapter command.
///
/// The fixture directory is expected to be the published `fixtures/adapter-protocol/0.1`
/// directory (or an external copy with the same versioned contents). Fixture payloads are
/// decoded through the existing Protocol/Public Contract canonical DTO parsers rather than
/// duplicated inside this crate.
#[derive(Debug, Clone)]
...
impl PublishedPositiveFixtureSuite {
    ...
    pub fn run(&self, command: ExternalAdapterCommand) -> ConformanceReport {
        ...
        let mut cases = vec![conformant_case(
            "adapter-protocol.schema-and-fixtures",
            ConformanceScope::Schema(PublishedContract::AdapterProtocol),
            "published Adapter Protocol 0.1 positive fixtures decoded and normalized through canonical DTO parsers",
        )];

        match fixtures.validate_public_contract_payloads() {
            Ok(()) => cases.push(conformant_case(
                "public-contract.reused-payloads",
                ConformanceScope::Schema(PublishedContract::PublicContract),
                "published validate/execute requests reuse canonical Public Contract target and MappingPlan payloads",
            )),
            Err(failure) => cases.push(harness_case(
                "public-contract.reused-payloads",
                ConformanceScope::Schema(PublishedContract::PublicContract),
                failure,
            )),
        ...
        let mut session = match self.harness.launch(command) {
            ...
            Ok(ExternalAdapterLaunch::ProtocolFailure(_)) => {
                cases.push(nonconformant_case(
                    ...
                    "positive fixture execution expected a successful adapter description but received ProtocolFailure",
                ));
                return report(cases);
            }
            Err(error) => {
        ...
        cases.push(conformant_case(
            "describe-adapter.positive",
            ConformanceScope::OperationSemantics(ProtocolOperation::DescribeAdapter),
            "external adapter completed version-safe describe_adapter bootstrap through the published transport boundary",
        ));
        ...
        cases.push(match session.validate_plan(&fixtures.validate_request) {
            ...
            Ok(AdapterProtocolObservation::ProtocolFailure(_)) => nonconformant_case(
                "validate-plan.positive",
                ConformanceScope::OperationSemantics(ProtocolOperation::ValidatePlan),
                "positive validate_plan fixture returned ProtocolFailure instead of a success response",
            ),
            ...
            Ok(AdapterProtocolObservation::Success(mut observed)) => {
                match observed.validate_against(&fixtures.execute_request) {
                    ...
                    Err(_) => nonconformant_case(
                        "execute-plan.positive",
                        ConformanceScope::OperationSemantics(ProtocolOperation::ExecutePlan),
                        "execute_plan response violated published request-relative execution invariants",
                    ),
                }
            }
            Ok(AdapterProtocolObservation::ProtocolFailure(_)) => nonconformant_case(
                "execute-plan.positive",
                ConformanceScope::OperationSemantics(ProtocolOperation::ExecutePlan),
                "positive execute_plan fixture returned ProtocolFailure instead of a success response",
            ),
            Err(error) => harness_case(
                "execute-plan.positive",
                ConformanceScope::OperationSemantics(ProtocolOperation::ExecutePlan),
...
impl PositiveFixtures {
    ...
    fn validate_public_contract_payloads(&self) -> Result<(), HarnessFailure> {
        self.validate_request
            .target
            .to_canonical_json()
            .map_err(|error| fixture_failure("validate request target", error))?;
        self.validate_request
            .plan
            .to_canonical_json()
            .map_err(|error| fixture_failure("validate request plan", error))?;
        self.execute_request
            .target
            .to_canonical_json()
            .map_err(|error| fixture_failure("execute request target", error))?;
        self.execute_request
            .plan
            .to_canonical_json()
            .map_err(|error| fixture_failure("execute request plan", error))?;
        Ok(())
    }
}
...
fn fixture_failure(subject: impl AsRef<Path>, error: impl Display) -> HarnessFailure {
    ...
    .expect("fixture failure detail is always non-blank")
}
